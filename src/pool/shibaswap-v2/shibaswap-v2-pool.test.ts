...
import { EvmChainId } from "~shibaswap-v3-sdk/chain/evm/index.js";
import { SHIBASWAP_V2_FACTORY_ADDRESS } from "~shibaswap-v3-sdk/config/features/shibaswap-v2.js";
import { Amount } from "~shibaswap-v3-sdk/currency/amount.js";
import { Price } from "~shibaswap-v3-sdk/currency/price.js";
import { Token } from "~shibaswap-v3-sdk/currency/token.js";
import { WETH9 } from "~shibaswap-v3-sdk/currency/tokens.js";
import { InsufficientInputAmountError } from "~shibaswap-v3-sdk/dex/errors.js";
import { computeShibaSwapV2PoolAddress } from "./compute-shibaswap-v2-pool-address.js";
import { ShibaSwapV2Pool } from "./shibaswap-v2-pool.js";
...
describe("computePairAddress", () => {
  it("should correctly compute the pool address", () => {
    ...
    const result = computeShibaSwapV2PoolAddress({
  ...
  it("should give same result regardless of token order", () => {
    ...
    let tokenA = USDC;
    let tokenB = DAI;
    const resultA = computeShibaSwapV2PoolAddress({
      factoryAddress: "0x1111111111111111111111111111111111111111",
      tokenA,
      tokenB,
    });
    ...
    tokenA = DAI;
    tokenB = USDC;
    const resultB = computeShibaSwapV2PoolAddress({
      factoryAddress: "0x1111111111111111111111111111111111111111",
      tokenA,
      tokenB,
    });
...
describe("Pair", () => {
  ...
  describe("#getAddress", () => {
    it("returns the correct address", () => {
      expect(ShibaSwapV2Pool.getAddress(USDC, DAI)).toEqual(
    ...
    it.skip("returns the default address for a testnet not in the map", () => {
      expect(() =>
        ShibaSwapV2Pool.getAddress(USDC_SEPOLIA, DAI_SEPOLIA)
      ).toEqual(
        computeShibaSwapV2PoolAddress({
          factoryAddress: SHIBASWAP_V2_FACTORY_ADDRESS[EvmChainId.SEPOLIA],
          tokenA: USDC_SEPOLIA,
          tokenB: DAI_SEPOLIA,
        })
      );
  ...
  describe("#priceOf", () => {
    const pair = new ShibaSwapV2Pool(
      Amount.fromRawAmount(USDC, "101"),
      Amount.fromRawAmount(DAI, "100")
    );
  ...
  describe("getInputAmount and getOutputAmount", () => {
    ...
    //       const pair = new ShibaSwapV2Pool(reserveBlasterAmount, reserveBlastAmount)
    ...
    describe("when NOT calculating FOT fees", () => {
      describe("getOutputAmount", () => {
        it("getOutputAmount for input token BLASTERS and output token BLAST", () => {
          ...
          expect(outputBlastAmount.toExact()).toEqual(
